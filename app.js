...
async function loadHomepageContent() {
    ...
    // --- Inject Quick Categories ---
    const quickSlider = document.getElementById('quickCategoriesSlider');
    if (quickSlider && quickSlider.children.length === 0) {
        const catIcons = {
            'Latest': 'fa-fire',
            'Hindi': 'fa-music',
            'Telugu': 'fa-headphones',
            'Tamil': 'fa-compact-disc',
            'BGM': 'fa-guitar',
            'Malayalam': 'fa-drum',
            'Bengali': 'fa-star',
        ...
        CATEGORIES.forEach(cat => {
            const icon = catIcons[cat] || 'fa-music';
            sliderHtml += `
                <div class="quick-cat-card" onclick="openCategoryView('${cat}')">
                    <i class="fas ${icon}"></i> <span>${cat}</span>
                </div>
            `;
        });
        quickSlider.innerHTML = sliderHtml;
    }
    // -------------------------------
